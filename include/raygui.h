...
//----------------------------------------------------------------------------------
    ...
    typedef struct Font {
        ...
        Rectangle2D *recs;        // Characters Rectangle2Ds in texture
...
RAYGUIDEF bool GuiWindowBox(Rectangle2D bounds, const char *title);                                       // Window Box control, shows a window that can be closed
RAYGUIDEF void GuiGroupBox(Rectangle2D bounds, const char *text);                                         // Group Box control with text name
RAYGUIDEF void GuiLine(Rectangle2D bounds, const char *text);                                             // Line separator control, could contain text
RAYGUIDEF void GuiPanel(Rectangle2D bounds);                                                              // Panel control, useful to group controls
RAYGUIDEF Rectangle2D GuiScrollPanel(Rectangle2D bounds, Rectangle2D content, Vector2 *scroll);               // Scroll Panel control

// Basic controls set
RAYGUIDEF void GuiLabel(Rectangle2D bounds, const char *text);                                            // Label control, shows text
RAYGUIDEF bool GuiButton(Rectangle2D bounds, const char *text);                                           // Button control, returns true when clicked
RAYGUIDEF bool GuiLabelButton(Rectangle2D bounds, const char *text);                                      // Label button control, show true when clicked
RAYGUIDEF bool GuiImageButton(Rectangle2D bounds, const char *text, Texture2D texture);                   // Image button control, returns true when clicked
RAYGUIDEF bool GuiImageButtonEx(Rectangle2D bounds, const char *text, Texture2D texture, Rectangle2D texSource);    // Image button extended control, returns true when clicked
RAYGUIDEF bool GuiToggle(Rectangle2D bounds, const char *text, bool active);                              // Toggle Button control, returns true when active
RAYGUIDEF int GuiToggleGroup(Rectangle2D bounds, const char *text, int active);                           // Toggle Group control, returns active toggle index
RAYGUIDEF bool GuiCheckBox(Rectangle2D bounds, const char *text, bool checked);                           // Check Box control, returns true when active
RAYGUIDEF int GuiComboBox(Rectangle2D bounds, const char *text, int active);                              // Combo Box control, returns selected item index
RAYGUIDEF bool GuiDropdownBox(Rectangle2D bounds, const char *text, int *active, bool editMode);          // Dropdown Box control, returns selected item
RAYGUIDEF bool GuiSpinner(Rectangle2D bounds, const char *text, int *value, int minValue, int maxValue, bool editMode);     // Spinner control, returns selected value
RAYGUIDEF bool GuiValueBox(Rectangle2D bounds, const char *text, int *value, int minValue, int maxValue, bool editMode);    // Value Box control, updates input text with numbers
RAYGUIDEF bool GuiTextBox(Rectangle2D bounds, char *text, int textSize, bool editMode);                   // Text Box control, updates input text
RAYGUIDEF bool GuiTextBoxMulti(Rectangle2D bounds, char *text, int textSize, bool editMode);              // Text Box control with multiple lines
RAYGUIDEF float GuiSlider(Rectangle2D bounds, const char *textLeft, const char *textRight, float value, float minValue, float maxValue);       // Slider control, returns selected value
RAYGUIDEF float GuiSliderBar(Rectangle2D bounds, const char *textLeft, const char *textRight, float value, float minValue, float maxValue);    // Slider Bar control, returns selected value
RAYGUIDEF float GuiProgressBar(Rectangle2D bounds, const char *textLeft, const char *textRight, float value, float minValue, float maxValue);  // Progress Bar control, shows current progress value
RAYGUIDEF void GuiStatusBar(Rectangle2D bounds, const char *text);                                        // Status Bar control, shows info text
RAYGUIDEF void GuiDummyRec(Rectangle2D bounds, const char *text);                                         // Dummy control for placeholders
RAYGUIDEF int GuiScrollBar(Rectangle2D bounds, int value, int minValue, int maxValue);                    // Scroll Bar control
RAYGUIDEF Vector2 GuiGrid(Rectangle2D bounds, float spacing, int subdivs);                                // Grid control


// Advance controls set
RAYGUIDEF int GuiListView(Rectangle2D bounds, const char *text, int *scrollIndex, int active);            // List View control, returns selected list item index
RAYGUIDEF int GuiListViewEx(Rectangle2D bounds, const char **text, int count, int *focus, int *scrollIndex, int active);      // List View with extended parameters
RAYGUIDEF int GuiMessageBox(Rectangle2D bounds, const char *title, const char *message, const char *buttons);                 // Message Box control, displays a message
RAYGUIDEF int GuiTextInputBox(Rectangle2D bounds, const char *title, const char *message, const char *buttons, char *text);   // Text Input Box control, ask for text
RAYGUIDEF Color GuiColorPicker(Rectangle2D bounds, Color color);                                          // Color Picker control (multiple color controls)
RAYGUIDEF Color GuiColorPanel(Rectangle2D bounds, Color color);                                           // Color Panel control
RAYGUIDEF float GuiColorBarAlpha(Rectangle2D bounds, float alpha);                                        // Color Bar Alpha control
RAYGUIDEF float GuiColorBarHue(Rectangle2D bounds, float value);                                          // Color Bar Hue control
...
static void DrawRectangleGradientEx(Rectangle2D rec, Color col1, Color col2, Color col3, Color col4);     // -- GuiColorPicker()
static void DrawTriangle(Vector2 v1, Vector2 v2, Vector2 v3, Color color);                              // -- GuiDropdownBox(), GuiScrollBar()
static void DrawTextureRec(Texture2D texture, Rectangle2D sourceRec, Vector2 position, Color tint);       // -- GuiImageButtonEx()

static void DrawTextRec(Font font, const char *text, Rectangle2D rec, float fontSize, float spacing, bool wordWrap, Color tint); // -- GuiTextBoxMulti()
//-------------------------------------------------------------------------------
...
static bool CheckCollisionPointRec(Vector2 point, Rectangle2D rec);   // Check if point is inside Rectangle2D
...
static void DrawRectangleGradientV(int posX, int posY, int width, int height, Color color1, Color color2);  // Draw Rectangle2D vertical gradient
//-------------------------------------------------------------------------------
...
static Rectangle2D GetTextBounds(int control, Rectangle2D bounds);  // Get text bounds considering control bounds
static const char *GetTextIcon(const char *text, int *iconId);  // Get text icon if provided and move text cursor

static void GuiDrawText(const char *text, Rectangle2D bounds, int alignment, Color tint);         // Gui draw text using default font
static void GuiDrawRectangle(Rectangle2D rec, int borderWidth, Color borderColor, Color color);   // Gui draw Rectangle2D using default raygui style
...
// Window Box control
bool GuiWindowBox(Rectangle2D bounds, const char *title)
{
    ...
    Rectangle2D statusBar = { bounds.x, bounds.y, bounds.width, (float)statusBarHeight };
    if (bounds.height < statusBarHeight*2.0f) bounds.height = statusBarHeight*2.0f;

    Rectangle2D windowPanel = { bounds.x, bounds.y + (float)statusBarHeight - 1, bounds.width, bounds.height - (float)statusBarHeight };
    Rectangle2D closeButtonRec = { statusBar.x + statusBar.width - GuiGetStyle(STATUSBAR, BORDER_WIDTH) - 20,
...
void GuiGroupBox(Rectangle2D bounds, const char *text)
{
    ...
    // Draw control
    //--------------------------------------------------------------------
    GuiDrawRectangle(RAYGUI_CLITERAL(Rectangle2D){ bounds.x, bounds.y, GROUPBOX_LINE_THICK, bounds.height }, 0, BLANK, Fade(GetColor(GuiGetStyle(DEFAULT, (state == GUI_STATE_DISABLED)? BORDER_COLOR_DISABLED : LINE_COLOR)), guiAlpha));
    GuiDrawRectangle(RAYGUI_CLITERAL(Rectangle2D){ bounds.x, bounds.y + bounds.height - 1, bounds.width, GROUPBOX_LINE_THICK }, 0, BLANK, Fade(GetColor(GuiGetStyle(DEFAULT, (state == GUI_STATE_DISABLED)? BORDER_COLOR_DISABLED : LINE_COLOR)), guiAlpha));
    GuiDrawRectangle(RAYGUI_CLITERAL(Rectangle2D){ bounds.x + bounds.width - 1, bounds.y, GROUPBOX_LINE_THICK, bounds.height }, 0, BLANK, Fade(GetColor(GuiGetStyle(DEFAULT, (state == GUI_STATE_DISABLED)? BORDER_COLOR_DISABLED : LINE_COLOR)), guiAlpha));

    GuiLine(RAYGUI_CLITERAL(Rectangle2D){ bounds.x, bounds.y, bounds.width, 1 }, text);
    //--------------------------------------------------------------------
}

// Line control
void GuiLine(Rectangle2D bounds, const char *text)
{
    ...
    // Draw control
    //--------------------------------------------------------------------
    if (text == NULL) GuiDrawRectangle(RAYGUI_CLITERAL(Rectangle2D){ bounds.x, bounds.y + bounds.height/2, bounds.width, 1 }, 0, BLANK, color);
    else
    {
        Rectangle2D textBounds = { 0 };
        ...
        GuiDrawRectangle(RAYGUI_CLITERAL(Rectangle2D){ bounds.x, bounds.y, LINE_TEXT_PADDING - 2, 1 }, 0, BLANK, color);
        GuiLabel(textBounds, text);
        GuiDrawRectangle(RAYGUI_CLITERAL(Rectangle2D){ bounds.x + LINE_TEXT_PADDING + textBounds.width + 4, bounds.y, bounds.width - textBounds.width - LINE_TEXT_PADDING - 4, 1 }, 0, BLANK, color);
    }
    //--------------------------------------------------------------------
}

// Panel control
void GuiPanel(Rectangle2D bounds)
{
...
Rectangle2D GuiScrollPanel(Rectangle2D bounds, Rectangle2D content, Vector2 *scroll)
{
    GuiControlState state = guiState;
    ...
    const Rectangle2D horizontalScrollBar = { (float)((GuiGetStyle(LISTVIEW, SCROLLBAR_SIDE) == SCROLLBAR_LEFT_SIDE)? (float)bounds.x + verticalScrollBarWidth : (float)bounds.x) + GuiGetStyle(DEFAULT, BORDER_WIDTH), (float)bounds.y + bounds.height - horizontalScrollBarWidth - GuiGetStyle(DEFAULT, BORDER_WIDTH), (float)bounds.width - verticalScrollBarWidth - 2*GuiGetStyle(DEFAULT, BORDER_WIDTH), (float)horizontalScrollBarWidth };
    const Rectangle2D verticalScrollBar = { (float)((GuiGetStyle(LISTVIEW, SCROLLBAR_SIDE) == SCROLLBAR_LEFT_SIDE)? (float)bounds.x + GuiGetStyle(DEFAULT, BORDER_WIDTH) : (float)bounds.x + bounds.width - verticalScrollBarWidth - GuiGetStyle(DEFAULT, BORDER_WIDTH)), (float)bounds.y + GuiGetStyle(DEFAULT, BORDER_WIDTH), (float)verticalScrollBarWidth, (float)bounds.height - horizontalScrollBarWidth - 2*GuiGetStyle(DEFAULT, BORDER_WIDTH) };

    // Calculate view area (area without the scrollbars)
    Rectangle2D view = (GuiGetStyle(LISTVIEW, SCROLLBAR_SIDE) == SCROLLBAR_LEFT_SIDE)?
                RAYGUI_CLITERAL(Rectangle2D){ bounds.x + verticalScrollBarWidth + GuiGetStyle(DEFAULT, BORDER_WIDTH), bounds.y + GuiGetStyle(DEFAULT, BORDER_WIDTH), bounds.width - 2*GuiGetStyle(DEFAULT, BORDER_WIDTH) - verticalScrollBarWidth, bounds.height - 2*GuiGetStyle(DEFAULT, BORDER_WIDTH) - horizontalScrollBarWidth } :
                RAYGUI_CLITERAL(Rectangle2D){ bounds.x + GuiGetStyle(DEFAULT, BORDER_WIDTH), bounds.y + GuiGetStyle(DEFAULT, BORDER_WIDTH), bounds.width - 2*GuiGetStyle(DEFAULT, BORDER_WIDTH) - verticalScrollBarWidth, bounds.height - 2*GuiGetStyle(DEFAULT, BORDER_WIDTH) - horizontalScrollBarWidth };
    ...
    {
        Rectangle2D corner = { (GuiGetStyle(LISTVIEW, SCROLLBAR_SIDE) == SCROLLBAR_LEFT_SIDE) ? (bounds.x + GuiGetStyle(DEFAULT, BORDER_WIDTH) + 2) : (horizontalScrollBar.x + horizontalScrollBar.width + 2), verticalScrollBar.y + verticalScrollBar.height + 2, (float)horizontalScrollBarWidth - 4, (float)verticalScrollBarWidth - 4 };
...
// Label control
void GuiLabel(Rectangle2D bounds, const char *text)
{
    GuiControlState state = guiState;
...
bool GuiButton(Rectangle2D bounds, const char *text)
{
    GuiControlState state = guiState;
    bool pressed = false;
...
bool GuiLabelButton(Rectangle2D bounds, const char *text)
{
    GuiControlState state = guiState;
    bool pressed = false;
...
bool GuiImageButton(Rectangle2D bounds, const char *text, Texture2D texture)
{
    return GuiImageButtonEx(bounds, text, texture, RAYGUI_CLITERAL(Rectangle2D){ 0, 0, (float)texture.width, (float)texture.height });
}

// Image button control, returns true when clicked
bool GuiImageButtonEx(Rectangle2D bounds, const char *text, Texture2D texture, Rectangle2D texSource)
{
    GuiControlState state = guiState;
    bool clicked = false;
...
bool GuiToggle(Rectangle2D bounds, const char *text, bool active)
{
    GuiControlState state = guiState;
...
int GuiToggleGroup(Rectangle2D bounds, const char *text, int active)
{
...
bool GuiCheckBox(Rectangle2D bounds, const char *text, bool checked)
{
    GuiControlState state = guiState;
    ...
    if (checked)
    {
        Rectangle2D check = { bounds.x + GuiGetStyle(CHECKBOX, BORDER_WIDTH) + GuiGetStyle(CHECKBOX, CHECK_PADDING),
...
int GuiComboBox(Rectangle2D bounds, const char *text, int active)
{
    GuiControlState state = guiState;

    bounds.width -= (GuiGetStyle(COMBOBOX, COMBO_BUTTON_WIDTH) + GuiGetStyle(COMBOBOX, COMBO_BUTTON_PADDING));

    Rectangle2D selector = { (float)bounds.x + bounds.width + GuiGetStyle(COMBOBOX, COMBO_BUTTON_PADDING),
...
bool GuiDropdownBox(Rectangle2D bounds, const char *text, int *active, bool editMode)
{
    GuiControlState state = guiState;
    ...
    if ((state != GUI_STATE_DISABLED) && !guiLocked && (itemsCount > 1))
    {
        ...
        if (editMode)
        {
            ...
            for (int i = 0; i < itemsCount; i++)
            {
                // Update item Rectangle2D y position for next item
...
bool GuiTextBox(Rectangle2D bounds, char *text, int textSize, bool editMode)
{
...
bool GuiSpinner(Rectangle2D bounds, const char *text, int *value, int minValue, int maxValue, bool editMode)
{
    GuiControlState state = guiState;
    ...
    Rectangle2D spinner = { bounds.x + GuiGetStyle(SPINNER, SPIN_BUTTON_WIDTH) + GuiGetStyle(SPINNER, SPIN_BUTTON_PADDING), bounds.y,
                          bounds.width - 2*(GuiGetStyle(SPINNER, SPIN_BUTTON_WIDTH) + GuiGetStyle(SPINNER, SPIN_BUTTON_PADDING)), bounds.height };
    Rectangle2D leftButtonBound = { (float)bounds.x, (float)bounds.y, (float)GuiGetStyle(SPINNER, SPIN_BUTTON_WIDTH), (float)bounds.height };
    Rectangle2D rightButtonBound = { (float)bounds.x + bounds.width - GuiGetStyle(SPINNER, SPIN_BUTTON_WIDTH), (float)bounds.y, (float)GuiGetStyle(SPINNER, SPIN_BUTTON_WIDTH), (float)bounds.height };
...
bool GuiValueBox(Rectangle2D bounds, const char *text, int *value, int minValue, int maxValue, bool editMode)
{
    ...
    if ((state == GUI_STATE_PRESSED) && (editMode && ((framesCounter/20)%2 == 0)))
    {
        ...
        Rectangle2D cursor = { bounds.x + GetTextWidth(textValue)/2 + bounds.width/2 + 2, bounds.y + 2*GuiGetStyle(VALUEBOX, BORDER_WIDTH), 1, bounds.height - 4*GuiGetStyle(VALUEBOX, BORDER_WIDTH) };
...
bool GuiTextBoxMulti(Rectangle2D bounds, char *text, int textSize, bool editMode)
{
    static int framesCounter = 0;           // Required for blinking cursor
    ...
    Rectangle2D cursor = { 0, 0, 1, (float)GuiGetStyle(DEFAULT, TEXT_SIZE) + 2 };
...
float GuiSliderPro(Rectangle2D bounds, const char *textLeft, const char *textRight, float value, float minValue, float maxValue, int sliderWidth)
{
    GuiControlState state = guiState;

    int sliderValue = (int)(((value - minValue)/(maxValue - minValue))*(bounds.width - 2*GuiGetStyle(SLIDER, BORDER_WIDTH)));

    Rectangle2D slider = { bounds.x, bounds.y + GuiGetStyle(SLIDER, BORDER_WIDTH) + GuiGetStyle(SLIDER, SLIDER_PADDING),
...
float GuiSlider(Rectangle2D bounds, const char *textLeft, const char *textRight, float value, float minValue, float maxValue)
{
    return GuiSliderPro(bounds, textLeft, textRight, value, minValue, maxValue, GuiGetStyle(SLIDER, SLIDER_WIDTH));
}

// Slider Bar control extended, returns selected value
float GuiSliderBar(Rectangle2D bounds, const char *textLeft, const char *textRight, float value, float minValue, float maxValue)
{
    return GuiSliderPro(bounds, textLeft, textRight, value, minValue, maxValue, 0);
}

// Progress Bar control extended, shows current progress value
float GuiProgressBar(Rectangle2D bounds, const char *textLeft, const char *textRight, float value, float minValue, float maxValue)
{
    GuiControlState state = guiState;

    Rectangle2D progress = { bounds.x + GuiGetStyle(PROGRESSBAR, BORDER_WIDTH),
...
// Status Bar control
void GuiStatusBar(Rectangle2D bounds, const char *text)
{
    GuiControlState state = guiState;
...
// Dummy Rectangle2D control, intended for placeholding
void GuiDummyRec(Rectangle2D bounds, const char *text)
{
    GuiControlState state = guiState;
...
int GuiScrollBar(Rectangle2D bounds, int value, int minValue, int maxValue)
{
    GuiControlState state = guiState;
    ...
    // Arrow buttons [<] [>] [∧] [∨]
    Rectangle2D arrowUpLeft = { 0 };
    Rectangle2D arrowDownRight = { 0 };

    // Actual area of the scrollbar excluding the arrow buttons
    Rectangle2D scrollbar = { 0 };

    // Slider bar that moves     --[///]-----
    Rectangle2D slider = { 0 };
    ...
    // Calculate Rectangle2Ds for all of the components
    arrowUpLeft = RAYGUI_CLITERAL(Rectangle2D){ (float)bounds.x + GuiGetStyle(SCROLLBAR, BORDER_WIDTH), (float)bounds.y + GuiGetStyle(SCROLLBAR, BORDER_WIDTH), (float)spinnerSize, (float)spinnerSize };

    if (isVertical)
    {
        arrowDownRight = RAYGUI_CLITERAL(Rectangle2D){ (float)bounds.x + GuiGetStyle(SCROLLBAR, BORDER_WIDTH), (float)bounds.y + bounds.height - spinnerSize - GuiGetStyle(SCROLLBAR, BORDER_WIDTH), (float)spinnerSize, (float)spinnerSize};
        scrollbar = RAYGUI_CLITERAL(Rectangle2D){ bounds.x + GuiGetStyle(SCROLLBAR, BORDER_WIDTH) + GuiGetStyle(SCROLLBAR, SCROLL_PADDING), arrowUpLeft.y + arrowUpLeft.height, bounds.width - 2*(GuiGetStyle(SCROLLBAR, BORDER_WIDTH) + GuiGetStyle(SCROLLBAR, SCROLL_PADDING)), bounds.height - arrowUpLeft.height - arrowDownRight.height - 2*GuiGetStyle(SCROLLBAR, BORDER_WIDTH) };
        ...
        slider = RAYGUI_CLITERAL(Rectangle2D){ (float)bounds.x + GuiGetStyle(SCROLLBAR, BORDER_WIDTH) + GuiGetStyle(SCROLLBAR, SCROLL_SLIDER_PADDING), (float)scrollbar.y + (int)(((float)(value - minValue)/range)*(scrollbar.height - sliderSize)), (float)bounds.width - 2*(GuiGetStyle(SCROLLBAR, BORDER_WIDTH) + GuiGetStyle(SCROLLBAR, SCROLL_SLIDER_PADDING)), (float)sliderSize };
    }
    else
    {
        arrowDownRight = RAYGUI_CLITERAL(Rectangle2D){ (float)bounds.x + bounds.width - spinnerSize - GuiGetStyle(SCROLLBAR, BORDER_WIDTH), (float)bounds.y + GuiGetStyle(SCROLLBAR, BORDER_WIDTH), (float)spinnerSize, (float)spinnerSize};
        scrollbar = RAYGUI_CLITERAL(Rectangle2D){ arrowUpLeft.x + arrowUpLeft.width, bounds.y + GuiGetStyle(SCROLLBAR, BORDER_WIDTH) + GuiGetStyle(SCROLLBAR, SCROLL_PADDING), bounds.width - arrowUpLeft.width - arrowDownRight.width - 2*GuiGetStyle(SCROLLBAR, BORDER_WIDTH), bounds.height - 2*(GuiGetStyle(SCROLLBAR, BORDER_WIDTH) + GuiGetStyle(SCROLLBAR, SCROLL_PADDING))};
        ...
        slider = RAYGUI_CLITERAL(Rectangle2D){ (float)scrollbar.x + (int)(((float)(value - minValue)/range)*(scrollbar.width - sliderSize)), (float)bounds.y + GuiGetStyle(SCROLLBAR, BORDER_WIDTH) + GuiGetStyle(SCROLLBAR, SCROLL_SLIDER_PADDING), (float)sliderSize, (float)bounds.height - 2*(GuiGetStyle(SCROLLBAR, BORDER_WIDTH) + GuiGetStyle(SCROLLBAR, SCROLL_SLIDER_PADDING)) };
    }
    ...
    if ((state != GUI_STATE_DISABLED) && !guiLocked)
    {
        ...
        if (CheckCollisionPointRec(mousePoint, bounds))
        {
            ...
            else if (IsMouseButtonDown(MOUSE_LEFT_BUTTON))
            {
                if (!isVertical)
                {
                    Rectangle2D scrollArea = { arrowUpLeft.x + arrowUpLeft.width, arrowUpLeft.y, scrollbar.width, bounds.height - 2*GuiGetStyle(SCROLLBAR, BORDER_WIDTH)};
                ...
                {
                    Rectangle2D scrollArea = { arrowUpLeft.x, arrowUpLeft.y+arrowUpLeft.height, bounds.width - 2*GuiGetStyle(SCROLLBAR, BORDER_WIDTH),  scrollbar.height};
...
// List View control
int GuiListView(Rectangle2D bounds, const char *text, int *scrollIndex, int active)
{
    int itemsCount = 0;
...
int GuiListViewEx(Rectangle2D bounds, const char **text, int count, int *focus, int *scrollIndex, int active)
{
    GuiControlState state = guiState;
    ...
    // Define base item Rectangle2D [0]
    Rectangle2D itemBounds = { 0 };
...
// Color Panel control
Color GuiColorPanelEx(Rectangle2D bounds, Color color, float hue)
{
    GuiControlState state = guiState;
    ...
    if (state != GUI_STATE_DISABLED)
    {
        ...
        Rectangle2D selector = { pickerSelector.x - GuiGetStyle(COLORPICKER, COLOR_SELECTOR_SIZE)/2, pickerSelector.y - GuiGetStyle(COLORPICKER, COLOR_SELECTOR_SIZE)/2, (float)GuiGetStyle(COLORPICKER, COLOR_SELECTOR_SIZE), (float)GuiGetStyle(COLORPICKER, COLOR_SELECTOR_SIZE) };
...
Color GuiColorPanel(Rectangle2D bounds, Color color)
{
...
float GuiColorBarAlpha(Rectangle2D bounds, float alpha)
{
    #define COLORBARALPHA_CHECKED_SIZE   10

    GuiControlState state = guiState;
    Rectangle2D selector = { (float)bounds.x + alpha*bounds.width - GuiGetStyle(COLORPICKER, HUEBAR_SELECTOR_OVERFLOW), (float)bounds.y - GuiGetStyle(COLORPICKER, HUEBAR_SELECTOR_OVERFLOW), (float)GuiGetStyle(COLORPICKER, HUEBAR_SELECTOR_HEIGHT), (float)bounds.height + GuiGetStyle(COLORPICKER, HUEBAR_SELECTOR_OVERFLOW)*2 };
    ...
    if (state != GUI_STATE_DISABLED)
    {
        ...
        for (int x = 0; x < checksX; x++)
        {
            ...
            {
                Rectangle2D check = { bounds.x + x*COLORBARALPHA_CHECKED_SIZE, bounds.y + y*COLORBARALPHA_CHECKED_SIZE, COLORBARALPHA_CHECKED_SIZE, COLORBARALPHA_CHECKED_SIZE };
...
float GuiColorBarHue(Rectangle2D bounds, float hue)
{
    GuiControlState state = guiState;
    Rectangle2D selector = { (float)bounds.x - GuiGetStyle(COLORPICKER, HUEBAR_SELECTOR_OVERFLOW), (float)bounds.y + hue/360.0f*bounds.height - GuiGetStyle(COLORPICKER, HUEBAR_SELECTOR_OVERFLOW), (float)bounds.width + GuiGetStyle(COLORPICKER, HUEBAR_SELECTOR_OVERFLOW)*2, (float)GuiGetStyle(COLORPICKER, HUEBAR_SELECTOR_HEIGHT) };
...
//      Color GuiColorPanel(Rectangle2D bounds, Color color)
//      float GuiColorBarAlpha(Rectangle2D bounds, float alpha)
//      float GuiColorBarHue(Rectangle2D bounds, float value)
// NOTE: bounds define GuiColorPanel() size
Color GuiColorPicker(Rectangle2D bounds, Color color)
{
    color = GuiColorPanel(bounds, color);

    Rectangle2D boundsHue = { (float)bounds.x + bounds.width + GuiGetStyle(COLORPICKER, HUEBAR_PADDING), (float)bounds.y, (float)GuiGetStyle(COLORPICKER, HUEBAR_WIDTH), (float)bounds.height };
    //Rectangle2D boundsAlpha = { bounds.x, bounds.y + bounds.height + GuiGetStyle(COLORPICKER, BARS_PADDING), bounds.width, GuiGetStyle(COLORPICKER, BARS_THICK) };
...
// Message Box control
int GuiMessageBox(Rectangle2D bounds, const char *title, const char *message, const char *buttons)
{
...
int GuiTextInputBox(Rectangle2D bounds, const char *title, const char *message, const char *buttons, char *text)
{
...
Vector2 GuiGrid(Rectangle2D bounds, float spacing, int subdivs)
{
    ...
    switch (state)
    {
        case GUI_STATE_NORMAL:
        {
            if (subdivs > 0)
            {
                ...
                {
                    Rectangle2D lineV = { bounds.x + spacing * i / subdivs, bounds.y, 1, bounds.height };
                ...
                {
                    Rectangle2D lineH = { bounds.x, bounds.y + spacing * i / subdivs, bounds.width, 1 };
    ...
    if (tryBinary)
    {
            ...
            (signature[3] == ' '))
        {
            ...
            if (fontDataSize > 0)
            {
                ...
                // Load font recs data
                font.recs = (Rectangle2D *)RAYGUI_CALLOC(font.charsCount, sizeof(Rectangle2D));
                for (int i = 0; i < font.charsCount; i++) fread(&font.recs[i], 1, sizeof(Rectangle2D), rgsFile);
                ...
                // Set font texture source Rectangle2D to be used as white texture to draw shapes
...
// Draw selected icon using Rectangle2Ds pixel-by-pixel
...
static Rectangle2D GetTextBounds(int control, Rectangle2D bounds)
{
...
static void GuiDrawText(const char *text, Rectangle2D bounds, int alignment, Color tint)
{
...
// Gui draw Rectangle2D using default raygui plain style with borders
static void GuiDrawRectangle(Rectangle2D rec, int borderWidth, Color borderColor, Color color)
{
    if (color.a > 0)
    {
        // Draw Rectangle2D filled with color
...
static bool CheckCollisionPointRec(Vector2 point, Rectangle2D rec)
{
...
// Draw Rectangle2D with vertical gradient fill color
// NOTE: This function is only used by GuiColorPicker()
static void DrawRectangleGradientV(int posX, int posY, int width, int height, Color color1, Color color2)
{
    Rectangle2D bounds = { (float)posX, (float)posY, (float)width, (float)height };
